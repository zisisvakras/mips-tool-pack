# an exam problem where pairs of consecutive characters in string must be flipped
# using the syscalls from this toolpack
...
main:
    ...
    jal print_string
    la $s0, buffer
    addi $s1, $s0, 19 #pos of the last char
loop:
    lbu $t0, 0($s0)
    lbu $t1, 1($s0)
    sb $t1, 0($s0)
